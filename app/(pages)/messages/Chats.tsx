            ...
            {messages?.map((msg: any) => (
                <div key={msg.id} className={`max-w-[70%] flex ${msg.senderId === currentUserId ? "self-end bg-[var(--voilet)] text-white" : "flex-row gap-5"}`}>
                    ...
                    <div className="relative overflow-hidden rounded-full w-[30px] h-[30px]">
                        <Image src={(msg.senderId !== currentUserId ? msg?.sender?.userImage : user?.userImage) || noProfile.src} alt='' fill className='absolute top-0 left-0 w-full h-full object-cover' />
                    </div>
                    {/* )} */}
                    ...
                    <div className={`w-full p-2 rounded-[20px] relative space-y-2 ${msg.senderId === currentUserId ? "bg-[var(--voilet)] text-white" : "bg-neutral-100"}`} >
                        ...
                        {/* Message text */}
                        <h5 className='min-w-[80px]'>{msg.text}</h5>

                        {/* Timestamp */}
                        <span className="absolute bottom-[-20px] left-0 text-xs font-semibold text-black">{moment(msg?.updatedAt).format("MMM D, h:mm A")}</span>
                    </div>
                </div>
            ))}
