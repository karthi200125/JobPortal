...
import UserBackImageForm from '@/app/Forms/UserImagesForm';
        ...
        <div className='relative w-full min-h-[200px] overflow-hidden rounded-[20px] border'>
            ...
            <div className='absolute top-0 left-0 w-full h-[100px] md:h-[200px]'>
                ...
                <Image
                    ...
                    width={150}
                    height={150}
                    className='w-[150px] h-[150px] absolute bottom-[-40px] left-5 rounded-full border-[4px] border-solid border-[var(--white)] object-cover'
                />
                {isCurrentUser && (
                    <Model
                        bodyContent={<UserBackImageForm />}
                        title='Edit Images'
            ...
            {isLoading ? (
                ...
                <div className='relative mt-[250px] w-full p-5 space-y-2'>
                    <h2 className='font-bold capitalize'>
                        {isOrg ? company?.companyName : profileUser?.username}
                    </h2>
                    <h3 className='w-[70%] text-lg text-[var(--lighttext)]'>
                        {isOrg ? company?.companyName : profileUser?.userBio}
                    </h3>
                    <h4 className='capitalize'>
                        {isOrg ? company?.companyCity : profileUser?.city},
                        {isOrg ? company?.companyState : profileUser?.state},
                        {isOrg ? company?.companyCountry : profileUser?.country}
                    </h4>
                    <div className='flex flex-row items-center gap-2 text-[var(--voilet)] hover:underline trans'>
                        <a
                            href={isOrg ? company?.companuWebsite : profileUser?.website || ''}
                            className='font-bold'
                        >
                            Personal Website
                        </a>
                    ...
                    {!isCurrentUser && (
                        <div className='flex flex-row items-center gap-5 mt-5'>
                            <Button
                                ...
                                isLoading={isPending}
                                className={
                                    isFollowings ? '!bg-[var(--voilet)] text-white' : ''
                                }
